pipeline {
  ...
  environment {
    IMAGE_NAME = 'payment-service' // diganti dengan nama image Docker yang akan dibuat
  ...
  stages {
    ...
    stage('Checkout Code') {
      steps {
        script {
          def repoUrl = 'https://github.com/Mini-Soccer-Project/payment-service.git' // diganti dengan URL repository Git
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/Mini-Soccer-Project/payment-service.git // diganti dengan URL repository Git (komentar ini nanti dihapus)
    ...
    stage('Deploy to Remote Server') {
      steps {
        script {
          def targetDir = "/home/faisalilhami/mini-soccer-project/payment-service" // diganti dengan direktori tempat menyimpan kode di server
          ...
          ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
            if [ -d "${targetDir}/.git" ]; then
                ...
                git clone -b "${TARGET_BRANCH}" git@github.com:Mini-Soccer-Project/payment-service.git "${targetDir}" // diganti dengan URL repository Git (komentar ini nanti dihapus)
                cd "${targetDir}"
            fi
